...
func main() {
	...
	// The select statement allows reading from multiple channels simultaneously.
	// Here, we use it to block until either the future is ready to read or the timer triggers, whichever happens first.
	get := func(s int) (result int, timedout bool) {
		select {
		case result = <-c3:
			return result, false
