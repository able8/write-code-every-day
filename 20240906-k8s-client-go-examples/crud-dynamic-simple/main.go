...
func main() {
	...
	// Read
	read, err := client.Resource(res).Namespace(namespace).Get(
		context.Background(),
		created.GetName(),
		metav1.GetOptions{},
	)
	...
	updated, err := client.Resource(res).Namespace(namespace).Update(
		context.Background(),
		read,
	...
	// Delete
	err = client.Resource(res).Namespace(namespace).Delete(
		context.Background(),
		created.GetName(),
